...
def get_files(dirname, filename_pat="*", recursive=False):
    ...
    for x in tf.io.gfile.listdir(dirname):
        ...
        elif fnmatch.fnmatch(x, filename_pat):#check whether the file is the behavior file
            files.append(path)
    return files
...
def get_worker_files(dirname,
    ...
    if shuffle:
        ...
        # g_mutex.acquire()
        random.seed(seed)#shuffle all data
        random.shuffle(all_files)
        # g_mutex.release()
    files = []
    for i in range(worker_rank, len(all_files), world_size):#start from worker_rank 
        files.append(all_files[i])#work rank
    logging.info(
...
class StreamReader:
    def __init__(self, data_paths, batch_size, shuffle=False, shuffle_buffer_size=1000):
        ...
        #1. Take # cycle_length elements from dataset A, 
        # apply map_func to these elements 
        # and get # cycle_length new datasets
        #2. Cycle through each returned dataset objects
        #  to produce # block_length new elements.
        #3. If all elements in one of the new datasets have been consumed
    ...
    def get_next(self):
        try:
            ret = self.session.run(self.next_batch)# use session to get next batch
